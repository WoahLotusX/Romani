do
	local ui = game:GetService("CoreGui"):FindFirstChild("whublib")
	if ui then
		ui:Destroy()
	end
end
...
local guicolor = Color3.fromRGB(59, 153, 253)

local whublib = Instance.new("ScreenGui")
whublib.Name = "whublib"
whublib.Parent = game.CoreGui
whublib.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	...
	function(io, p)
		if io.KeyCode == Enum.KeyCode.RightShift then
			if uitoggled == false then
				whublib.Enabled = false
				uitoggled = true
			else
				whublib.Enabled = true
				uitoggled = false
			end
		end
	end
...
function lib:notification(textt)
	for i, v in next, whublib:GetChildren() do
	...
	notificationframe.Parent = whublib
...
function lib:MainWindow(text)
    ...
    if makefolder and isfolder and isfile and readfile and writefile then
        if not isfolder("wotushub") then
            makefolder("wotushub")
        end
    
        if not isfolder("wotushub/" .. text) then
            makefolder("wotushub/" .. text)
        end
    
        if isfile("wotushub/color_data.json") then
            local data = HttpService:JSONDecode(readfile("wotushub/color_data.json"))
            guicolor = Color3.new(unpack(data))
        else
            writefile("wotushub/color_data.json", HttpService:JSONEncode({guicolor.R,guicolor.G,guicolor.B}))
        end
    end
	...
	function window:Tab(title)
		...
		function tab:Colorpicker(text, presetcolor, callback)
            ...
            function returns:Set(colorv)
                currentcolor.BackgroundColor3 = colorv
                color.BackgroundColor3 = colorv
                callback(color.BackgroundColor3)
            end

            function returns:SetRainbow(value)
                RainbowColorPicker = value
		...
		function tab:Slider(text, min, max, def, callback)
			...
			slidingframe.BackgroundColor3 = guicolor
            ...
            function returns:Set(newvalue)
                slidingframe:TweenSize(UDim2.new(math.clamp(newvalue / max, 0, 1), 0, 1, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true)
			...
			local function slide(input)
				...
				local num = math.floor(((pos.X.Scale * max) / max) * (max - min) + min)
		...
		function tab:Textbox(text, disapper, default, callback)
			local returns = {}
		...
		function tab:Dropdown(name, list, callback)
			...
			function returns:SetList(newlist)
                list = newlist
            ...
            function returns:Set(value)
                if findintable(list, value) then
    ...
    function window:SettingsTab(callbacks)
        local Tab = self:Tab("Settings")

        Tab:Colorpicker("GUI Color", guicolor, function(color)
            guicolor = color
            writefile("wotushub/color_data.json", HttpService:JSONEncode({color.R, color.G, color.B}))
        end)

        Tab:Button("Unload", function()
            local ui = game:GetService("CoreGui"):FindFirstChild("whublib")
            if ui then
                ui:Destroy()
            end
            if callbacks.unload then
                callbacks.unload()
            end
        end)
        ...
        local GetConfigs = function()
            ...
            for i,v in pairs(listfiles("wotushub/" .. text)) do
                print(i,v)
                if isfile(v) and tostring(v):find(".json") then
                    local name = tostring(v):gsub("wotushub/" .. text .. "\\", "")
                    table.insert(Confs, i, name)
                end
            end
        ...
        Tab:Textbox("Config Name", false, function(text)
            ConfigName = text
        end)

        local ConfigList = Tab:Dropdown("Config", GetConfigs(), function(text)
            SelectedConfig = text
        end)

        Tab:Button("Create", function()
            if callbacks.create then
                callbacks.create(ConfigName)
            end
        end)

        Tab:Button("Load", function()
            if callbacks.load then
                callbacks.load(SelectedConfig)
            end
        end)

        Tab:Button("Save", function()
            if callbacks.save then
                callbacks.save(SelectedConfig)
            end
        end)

        Tab:Button("Delete", function()
            if callbacks.delete then
                callbacks.delete(SelectedConfig)
            end
        end)

        Tab:Button("Refresh", function()
            ConfigList:Refresh()
            ConfigList:SetList(GetConfigs())
